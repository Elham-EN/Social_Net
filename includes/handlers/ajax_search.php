<?php
    ...
    /*
        reece kenny in search input field
        [0] reece
        [1] kenney
    */ 
    $names = explode(" ", $query);  
    ...
    if($query != "") { //if query not empty
        while($row = mysqli_fetch_array($usersReturnedQuery)) {
            $user = new User($con, $userLoggedIn);
            if ($row['username'] != $userLoggedIn) //if user has not found itself in search result
                $mutual_friends = $user->getMutualFriends($row['username']) . " friends in common";
            else //else if user did find itself
                $mutual_friends == "";
            //Returned String
